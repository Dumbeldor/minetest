	...
	if (event.EventType == EET_MOUSE_INPUT_EVENT &&
		...
		if (updown == 0) {
			...
			if (s.isValid() && s.listname == "craftpreview") {
				...
				if (s.isValid()) {
					// Clicked a slot: move
					if (event.MouseInput.Shift && button == 1) {
						ISplitAction *a = new ISplitAction();
						...
						int nb = 1;
						int total = 0;
						std::vector<s16> case_present = {};
						ItemStack &source = inv_selected->getList(m_selected_item->listname)->getItem(m_selected_item->i);
						for (s16 i = 0; i < list_s->getSize(); i++) {
							//if (list_s->getItem(i).name == )
							ItemStack &item = list_s->getItem(i);
							if (item.name == source.name && item.count > 0) {
								if (m_selected_amount > 1) {
									case_present.push_back(i);
									move_amount = m_selected_amount - 1;
									m_selected_amount = 1;
									total += move_amount;
						...
						//total += m_selected_amount;
						int repart = total / nb;
						if (total == 0)
							move_amount = m_selected_amount - 1;
						else {
							move_amount = 0;
							for (int i = 0; i < list_s->getSize(); i++) {
								ItemStack &item = list_s->getItem(i);
								if (item.name == source.name && item.count > 0) {
									if (item.count > repart) {
										IMoveAction *a = new IMoveAction();
										int test = list_s->getItem(i).count - repart;
										a->count = test;
